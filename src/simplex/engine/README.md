...
- `engine.code` -- `code_block(code, language=...)` wrapping modern `manim.Code`; `highlight_code_lines`, `code_explain`, `transform_code_lines`; `DarculaStyle` + `register_darcula()`
- `engine.geometry` -- `get_convex_hull_polygon(points)` (needs `simplex[geometry]`), `get_surrounding_rectangle(a, b)`, `get_frame_center(...)`
...
- Don't re-implement Dastimator's `compile_code_tex` here. The modern `manim.Code` no longer exposes `code_json` / `tab_spaces`; LaTeX-in-code is out of scope until upstream offers a hook.
