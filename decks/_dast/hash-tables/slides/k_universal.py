...
class KUniversalHashFamilies(BaseSlide):
    def construct(self) -> None:
        ...
        hash_model = (
            VGroup(hash_family_text, keys_text, array_text, select_text, select_arrow, hash_family, ht)
            .scale(0.8)
...
\underset{h\in\mathcal{H}}{\mathbb{P}}\left[h\left(y_{1}\right)=i_{1}\land\ldots\land h\left(y_{k}\right)=i_{k}\right]\leq\frac{1}{m^{k}}
        ...
        prob_eq.scale_to_fit_width(config.frame_width * 0.9).next_to(
            definition[0][:prob_eq_start], DOWN, buff=0.3,
        ).set_x(0)
        color_tex(definition, {r"\textbf{-universal}": YELLOW}, tex_class=MathTex)
        color_tex(
            definition,
            {"y_{1}": SELECT_KEY_COLOR, "y_{k}": SELECT_KEY_COLOR, "h": FUNCS_COLOR, "k": K_UNIVERSAL_COLOR},
            tex_class=MathTex,
        )
        definition[0][-1].set_color(K_UNIVERSAL_COLOR)
        whole_family = VGroup(ht, select_arrow, select_text, hash_family)
        self.play(
            Unwrite(array_text), Unwrite(keys_text), Unwrite(hash_family_text), Unwrite(reminder),
        ...
        self.next_slide()
        self.play(
            two_universal_def.animate.scale(0.7).next_to(title, DOWN, buff=0.5).set_x(frame_center[0]),
        )
        ...
        self.next_slide()
        down_buff = 0.4
        down_arrow = Tex(r"$\Downarrow$").scale(1.3).next_to(two_universal_def, DOWN, buff=down_buff)
        ...
        for_i = Tex(r"for ", "$i_1=$", "$3$", " , ", "$i_2=$", "$2$", r"\ \ :").next_to(for_x_y, DOWN)
        first_i = for_i[2]
        second_i = for_i[5]
        ...
        prob_sq = (
            ...
            .match_height(prob_paren).scale(0.9).move_to(prob_paren)
        )
                ...
                for i_1 in range(array_size):
                    for i_2 in range(array_size):
                        if i_1 == array_size - 2 or i_2 == array_size - 2:
                            continue
                        anims = animate_collision_prob(y_1, y_2, hashs_tab, bg_cells, i_1=i_1, i_2=i_2)
                        new_y1 = MathTex(
                            rf"x_{{{LAST_IDX if y_1 == keys_size - 1 else y_1 + 1}}}",
                            color=SELECT_KEY_COLOR,
                        ).move_to(first_key).align_to(first_key, LEFT)
                        new_y2 = MathTex(
                            rf"x_{{{LAST_IDX if y_2 == keys_size - 1 else y_2 + 1}}}",
                            color=SELECT_KEY_COLOR,
                        ).move_to(second_key).align_to(second_key, LEFT + UP)
                        new_i1 = MathTex(
                            str(i_1 + 1) if i_1 != array_size - 1 else "m",
                        ).move_to(first_i).align_to(first_i, LEFT)
                        new_i2 = MathTex(
                            str(i_2 + 1) if i_2 != array_size - 1 else "m",
                        ).move_to(second_i).align_to(second_i, LEFT)
                        self.play(
                            *anims,
        ...
        self.next_slide()
        to_unwrite = [
            down_arrow, for_x_y, for_i, alt_def, prob_sq,
            first_key, second_key, first_i, second_i, two_universal_def, bg_cells,
        ]
