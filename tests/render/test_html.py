...
def test_render_html_writes_slides_html(tmp_path: Path) -> None:
    ...
    manifest = DeckManifest(
        deck_slug=deck.slug,
        slides=(
            SlideRef(index=0, scene="S1", title="Hello"),
            SlideRef(index=1, scene="S1"),
        ),
    )
    out = tmp_path / "out"
    out.mkdir()
...
def test_render_html_copies_videos_when_present(tmp_path: Path) -> None:
    ...
    manifest = DeckManifest(
        deck_slug=deck.slug,
        slides=(SlideRef(index=0, scene="S1", title="Hi", video_paths=(fake_video,)),),
    )
    out = tmp_path / "out"
    out.mkdir()
    render_html(deck, manifest, output_dir=out, static_prefix="/static")
    assert (out / "segments" / "0000.mp4").exists()
    html = (out / "slides.html").read_text(encoding="utf-8")
    assert "segments/0000.mp4" in html
