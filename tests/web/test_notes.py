...
def test_fenced_code_block_survives() -> None:
    ...
    # Pygments wraps tokens in styled <span>s; check that the literal made
    # it through (the integer 1 is rendered as a colored span).
    assert "x =" in html
    assert ">1<" in html


def test_fenced_code_block_uses_darcula_keyword_color() -> None:
    html = render_text("```python\ndef f(): pass\n```\n")
    # Darcula renders Python keywords in #CC7832 -- if the style ever drifts,
