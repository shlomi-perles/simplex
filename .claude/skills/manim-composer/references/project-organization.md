...
my_video/
  ...
  requirements.txt    # python -m manim + any extra deps
```
...
class S01Intro(Scene):
    def construct(self) -> None:
        ...
        VGroup(title, subtitle).arrange(DOWN, buff=MED_LARGE_BUFF)
...
for scene_file in scenes/s*.py; do
    ...
    python -m manim "$QUALITY" --fps "$FPS" --format "$FORMAT" "$scene_file"
done
...
3. **Test at low quality first**: `python -m manim -ql scenes/s01_intro.py` before rendering at high quality.
4. **Use `python -m manim -s`** to render just the last frame when debugging layout.
