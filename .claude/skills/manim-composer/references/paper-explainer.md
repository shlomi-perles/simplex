    ...
    def construct(self) -> None:
        ...
        for tex_str, description, color in terms:
            ...
            for part in parts:
                part.set_opacity(1)
            first_part = parts[0]
            rect = SurroundingRectangle(first_part, color=color)
            brace = Brace(first_part, DOWN, color=color)
            label = Text(description, font_size=22, color=color)
...
class PipelineDiagram(Scene):
    ...
    def construct(self) -> None:
        ...
        for name, color in stage_data:
            ...
            boxes.add(box)
        boxes.arrange(RIGHT, buff=LARGE_BUFF)
