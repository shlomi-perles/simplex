...
### ValueTracker arithmetic (v0.19.1+)

ValueTracker overloads `+`, `-`, `*`, `/`, `//`, `%`, and `**` so you can build derived trackers without writing updaters. Each operator returns a NEW `ValueTracker` whose value is the result.

```python
t = ValueTracker(2)
double = t * 2                 # ValueTracker(4)
shifted = t + 5                # ValueTracker(7)
squared = t ** 2               # ValueTracker(4)
```

In-place forms (`+=`, `-=`, `*=`, `/=`, `//=`, `%=`, `**=`) mutate the tracker in place — equivalent to `t.increment_value(...)`.

This is convenient for one-shot derived values, but the binary-operator result is a *snapshot* — animating `t` does not propagate through `t * 2`. For continuous propagation, use an updater that re-reads `t.get_value()`.

### Scene.time (v0.19+)

`self.time` returns the current scene time in seconds — useful inside updaters that want absolute time without threading their own clock:

```python
square = Square()
square.add_updater(lambda m: m.set_color(interpolate_color(BLUE, RED, (np.sin(self.time) + 1) / 2)))
self.add(square)
...
## Mobject.always: concise updater sugar (v0.20+)

`Mobject.always` lets you call a method every frame without writing a lambda.
It calls the method immediately, then attaches an updater that keeps it true.
If you need to delay the first call, use `add_updater` instead.

```python
sq = Square().to_edge(LEFT)
label = Text("Follow")

# label stays above the square as it moves
label.always.next_to(sq, UP, buff=0.2)
...
During `self.wait()`, Manim decides whether the frame is static when `frozen_frame=None`. Use `frozen_frame=False` when you need ordinary updaters to keep running during a pause. For a truly frozen frame:

```python
self.wait(2, frozen_frame=True)   # no updaters fire
self.wait(2, frozen_frame=False)  # force updaters to run
```

## turn_animation_into_updater + delay (v0.19+)

`turn_animation_into_updater(anim, delay=0.5)` lets you start a finite animation as a background updater after a delay -- handy for synchronizing parallel motion without nesting `Succession`/`AnimationGroup`.

```python
self.add(square)
turn_animation_into_updater(square.animate.to_edge(RIGHT, run_time=2), delay=1.0)
...
class LiveComputation(Scene):
    def construct(self) -> None:
        ...
        ).arrange(DOWN, aligned_edge=LEFT, buff=MED_LARGE_BUFF).shift(LEFT * 2)
