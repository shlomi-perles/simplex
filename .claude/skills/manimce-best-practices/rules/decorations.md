...
- `SurroundingRectangle` accepts **multiple Mobjects** positionally — it draws a single rect around the union: `SurroundingRectangle(eq[0], eq[2], color=YELLOW)`.
- `buff` accepts a `(buff_x, buff_y)` tuple for asymmetric padding: `buff=(0.4, 0.1)`.
- All non-Mobject parameters MUST be passed as keyword arguments now (`color=...`, `buff=...`, `corner_radius=...`).
- Default color is now `PURE_YELLOW`.
...
both = SurroundingRectangle(
    eq.get_part_by_tex("F"),
    eq.get_part_by_tex("a"),
    color=YELLOW,
    buff=(0.2, 0.1),         # wider horizontally, tighter vertically
)
```
...
# Use the dedicated helpers instead:
brace_label = BraceLabel(group, r"n = 3", font_size=28)        # MathTex label
brace_text  = BraceText(group, "Three squares", font_size=24)  # Text label
self.play(brace_label.creation_anim())   # bundles GrowFromCenter + FadeIn
self.play(brace_text.creation_anim())

# Or build the label yourself and place it manually
label = Tex(r"n = 3", font_size=28)
brace.put_at_tip(label)
self.play(Create(brace), Write(label))
```

`BraceLabel` defaults to `MathTex` for the label, `BraceText` defaults to `Text`. Both expose `.brace`, `.label`, `.shift_brace(new_obj)`, `.change_label(new_text)`, and `.change_brace_label(new_obj, new_text)`.
...
## Labeled lines and arrows (v0.18+)

`LabeledLine` / `LabeledArrow` bundle a line with a label that sits on it -- saves the manual `next_to`+`add` dance.

```python
ll = LabeledLine(
    label="L = 4",
    label_position=0.5,         # 0..1 along the line
    start=LEFT * 2,
    end=RIGHT * 2,
    label_config={"font_size": 28, "color": YELLOW},
)
la = LabeledArrow(
    label=MathTex(r"\vec{F}"),
    start=ORIGIN,
    end=RIGHT * 3,
)
```

`LabeledPolygram` (v0.19+) places a label centered inside an arbitrary polygon. `Label` (v0.19+) is a labeled rounded box, useful for diagram nodes.
