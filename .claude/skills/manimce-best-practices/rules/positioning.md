...
class NextToExample(Scene):
    def construct(self):
        ...
        # With custom spacing
        triangle.next_to(square, DOWN, buff=MED_LARGE_BUFF)
...
# Custom buffer: use constants or a meaningful numeric value.
circle.next_to(square, RIGHT, buff=0)               # No gap
circle.next_to(square, RIGHT, buff=LARGE_BUFF)      # Large gap
circle.next_to(square, RIGHT, buff=MED_LARGE_BUFF)  # 0.5-unit gap
```
...
class CornerExample(Scene):
    def construct(self):
        ...
        # Default corner buffer is already MED_LARGE_BUFF.
        t5 = Text("Buffered").to_corner(UL)
```
