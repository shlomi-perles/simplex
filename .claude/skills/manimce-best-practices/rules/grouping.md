...
class ArrangeExample(Scene):
    def construct(self):
        ...
        row.arrange(RIGHT, buff=MED_LARGE_BUFF).shift(UP * 2)

        # Vertical arrangement
        column = VGroup(*[Square().scale(0.3) for _ in range(4)])
        column.arrange(DOWN, buff=MED_LARGE_BUFF).shift(LEFT * 2)
...
class GridExample(Scene):
    def construct(self):
        ...
        # With spacing
        grid.arrange_in_grid(rows=3, cols=4, buff=MED_LARGE_BUFF)
