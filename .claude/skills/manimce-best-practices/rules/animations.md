...
| `AddTextWordByWord(text)` | Word-by-word text reveal; check the current docs/issues before relying on it in production |
...
**Typing animations (v0.19+)** -- letter-by-letter reveal with a cursor:

```python
text = Text("Hello").to_edge(LEFT)
cursor = Rectangle(height=text.height, width=0.1, fill_opacity=1, color=WHITE)
self.play(TypeWithCursor(text, cursor, time_per_char=0.05))
self.play(Blink(cursor, blinks=2))           # blink helper
self.play(UntypeWithCursor(text, cursor))    # erase
```

Use `AddTextLetterByLetter` / `RemoveTextLetterByLetter` if you don't need a cursor.

**`Add(mob)` (v0.19+)** -- a zero-runtime "add to scene" wrapped as an animation. Useful inside `Succession` / `LaggedStart` when you want a mob to appear with no transition:

```python
self.play(Succession(
    FadeIn(intro),
    Wait(1.0),
    Add(detail_a),                 # instant
    Add(detail_b, run_time=0.2),   # appears after 0.2s pause
    FadeOut(intro),
))
```
...
## Class-wide defaults with set_default (v0.19+)

`Animation.set_default(**kwargs)` (class method) lets you override defaults for an animation class once. The change persists for the rest of the Python session — call with no args to reset.

```python
class Demo(Scene):
    def construct(self):
        FadeIn.set_default(shift=UP * 0.4, run_time=0.6)
        Write.set_default(run_time=1.5)

        self.play(FadeIn(Text("A")))   # uses shift=UP*0.4, run_time=0.6
        self.play(Write(MathTex("x")))  # uses run_time=1.5

        FadeIn.set_default()            # reset back to library default
```

Prefer this over wrapping every `play()` with the same kwargs.
...
| `smoothstep` (v0.18+) | Cubic Hermite, sharper | UI-style transitions |
| `smootherstep` (v0.18+) | Quintic, even sharper | Polished motion graphics |
| `smoothererstep` (v0.18+) | Septic, sharpest still | Ultra-clean inertia |
