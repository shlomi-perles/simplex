...
Use `Sector(radius=..., angle=...)` for a pie slice. Do not pass `inner_radius` or `outer_radius` to `Sector`; use `AnnularSector` when you need both radii.
...
Use `TangentialArc` to draw an arc tangent to two lines meeting at a corner.

```python
class TangentialArcExample(Scene):
    def construct(self):
        line1 = Line(LEFT * 3, RIGHT)
        line2 = Line(ORIGIN, UP * 3).shift(RIGHT)
        arc = TangentialArc(
            line1,
            line2,
            radius=0.75,
