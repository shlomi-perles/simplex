...
### Using tex_to_color_map

When colors are known at construction time, pass the mapping directly:

```python
eq = MathTex(
    r"\vec{F} = m \vec{a}",
    tex_to_color_map={r"\vec{F}": BLUE, "m": GREEN, r"\vec{a}": YELLOW},
)
```
...
ManimCE 0.20.x handles `MathTex` splitting more robustly than older versions, but the best animation pattern is still to split meaningful terms explicitly or isolate repeated substrings:
