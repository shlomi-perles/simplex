...
**Cause:** `self.wait()` uses `frozen_frame=None` by default. Manim treats a wait as static unless it detects scene updaters, time-based mobject updaters, or a stop condition. Plain updaters without `dt` may not run during the wait.

**Fix:** Use `self.wait(frozen_frame=False)` whenever visible objects must continue updating during a wait.
...
# BAD: t.get_value() captured at attach time, dot never moves
dot.always.move_to(RIGHT * t.get_value())
...
**Cause:** `brace.get_text("label", font_size=24)` crashes because `get_text` forwards `**kwargs` to `put_at_tip` -> `next_to`, which does not accept `font_size`.

**Fix:** Use the built-in `BraceLabel`/`BraceText` (which take `font_size` directly), or create the label first:
```python
# BAD
label = brace.get_text("label", font_size=24)

# GOOD: BraceLabel = brace + MathTex label; BraceText = brace + Text label
brace_label = BraceLabel(obj, r"L = 4", font_size=24)        # MathTex
brace_text  = BraceText(obj, "Three squares", font_size=24)  # plain Text
self.play(brace_label.creation_anim())  # builds GrowFromCenter + FadeIn
...
When `self.wait()` is effectively static, Manim reuses a frozen frame for performance. This is correct for ordinary pauses, but wrong when an updater should keep changing the scene.

```python
# BAD: updater may stop during wait
self.wait(3)
...
python -m manim does not wrap text. A long `Text(...)` or `MathTex(...)` simply extends off-screen. You must set `font_size` small enough or manually break lines with `\n`.
...
```bash
python -m manim -pql scene.py MyScene    # 480p, 15fps - very fast
```
...
## Removed / renamed APIs (v0.18 - v0.20)

If you see code from older tutorials using these, replace them:

| Old (pre-v0.18)                          | Modern (v0.20.x)                                 |
|------------------------------------------|--------------------------------------------------|
| `Mobject.wag(...)`                       | removed in v0.18.1 — use `Wiggle` animation      |
| `manim new` CLI                          | removed in v0.18.1 -- start scenes manually       |
| `Code(code="...", language=...)` (old)   | `Code(code_string="...", language=...)` (v0.19)  |
| `Code.styles_list` (attribute)           | `Code.get_styles_list()` (method, v0.19)         |
| `Sector(inner_radius=, outer_radius=)`   | `Sector(radius=...)` or `Annulus`/`AnnularSector`|
| `ManimColor.from_hex(hex=...)`           | `ManimColor.from_hex(hex_str=...)` (v0.19)       |
| `Scene.next_section(type=...)`           | `Scene.next_section(section_type=...)` (v0.19)   |
| `SceneFileWriter.save_final_image()`     | `SceneFileWriter.save_image()` (v0.19.1)         |
| `colour.Color(...)` (external dep)       | `ManimColor("#1F77B4")` (v0.18 rewrite)          |
| `lanczos`/`box`/`hamming` resampling     | removed in v0.20.0 from `ImageMobject`           |

The `colour` library is no longer a dependency — never `import colour` in Manim code.

## Performance tips

| Situation | Fast approach |
|---|---|
| Development iteration | `python -m manim -pql` (480p, 15fps) |
...
| Final render | `python -m manim -qh` (1080p) or `python -m manim -qk` (4K) |
