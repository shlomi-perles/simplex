...
**Sphere** -- a ball. `radius` defaults to 1. `resolution=(u, v)` controls
smoothness: more segments means smoother but slower to render. Default (20, 20)
is fine for most uses.

```python
sphere = Sphere(resolution=(20, 20)).set_color(BLUE)
```

**Cube** -- a box with equal sides. `side_length` defaults to 2. Set
`fill_opacity < 1` to see through faces.

```python
cube = Cube(fill_color=GREEN, fill_opacity=0.5)
```
...
**Dot3D** -- a small sphere used as a 3D point marker (radius defaults to
`DEFAULT_DOT_RADIUS`).

```python
dot = Dot3D(point=[1, 2, 3], color=RED)
```
...
Wrap an arbitrary point cloud in its convex polyhedron — useful for "set of measurements" envelopes or when constructing diagrams from raw vertex lists.

```python
import numpy as np

points = np.random.RandomState(0).randn(40, 3)
hull = ConvexHull3D(*points, faces_config={"fill_opacity": 0.4, "fill_color": BLUE})
self.add(hull)
```

For 2D usage, see [`ConvexHull` in mobjects.md](mobjects.md). Both replace what previously required `scipy.spatial.ConvexHull` plus manual mobject construction.

## ImageMobject in 3D (v0.20 changes)

`ImageMobject` 3D rotation/flipping was fixed in v0.20. Older `lanczos`, `box`, and `hamming` resampling options were removed, so prefer the default image settings unless a render proves you need a supported override.
