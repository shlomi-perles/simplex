...
### Color-by-value with `colorscale` (v0.19+)

`axes.plot(..., colorscale=[...])` colors the curve by its y-value (default) or x-value. Pass a list of colors or `(color, pivot)` tuples for explicit transition points.

```python
# Color along the y axis
g1 = axes.plot(np.sin, x_range=[-PI, PI], colorscale=[BLUE, GREEN, YELLOW, RED])

# Explicit pivots: red below 0, green above 0
g2 = axes.plot(lambda x: x**3 - x, colorscale=[(RED, -1), (GREEN, 1)])

# Switch to color along x axis
g3 = axes.plot(np.cos, colorscale=[BLUE, RED], colorscale_axis=0)
```

For 3D plots, `Surface.set_fill_by_value(axes, colorscale=[...], axis=2)` uses the same color-scale idea. Cairo `Surface` uses `axis=2`; OpenGL-specific `OpenGLSurface` accepts `colorscale_axis=2` in its constructor.
...
# v0.19+ shorthand: @ operator
point = axes @ (2, 3)                 # same as axes.coords_to_point(2, 3)
point = axes @ some_mob               # uses some_mob.get_center()

# Data coordinates from scene point
x, y = axes.point_to_coords(point)    # or axes.p2c(point)
x, y = point @ axes                   # v0.19+ reverse @ (rmatmul)

# Place a dot at data coordinates
dot = Dot(axes @ (1, 1), color=RED)
```

**Note:** `c2p` (v0.20+) correctly accepts flat lists and 1D arrays in addition to plain `(x, y)` tuples — handy when forwarding numpy data.
...
dot = always_redraw(
    lambda: Dot(
        axes @ (t.get_value(), np.sin(t.get_value())),  # v0.19+ shorthand
        color=RED,
    )
)
self.add(dot)
