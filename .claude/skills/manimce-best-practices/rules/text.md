...
Use `Code` for syntax-highlighted source snippets. In ManimCE 0.20.1, pass either `code_file=` or `code_string=`, and use `language=` when the language cannot be inferred.

```python
class CodeExample(Scene):
    def construct(self):
        code = Code(
            code_string="def square(x):\n    return x * x",
            language="python",
...
4. **Use Code for source snippets** - Avoid hand-building code with many Text lines
5. **Install fonts system-wide** - Manim uses system fonts
6. **Keep font_size consistent** - Use the same size for related text
